...
const createMarkdownStyle = (_theme) => {
	const theme = {
		...defaultTheme,
		..._theme
	};
	return {
		...
		code: {
			...
			run: {
				font: "Courier New",
				size: theme.codeSize * 2,
				color: theme.code
			},
			paragraph: {
				shading: { fill: theme.codeBackground },
				border: {
					top: {
						style: "single",
						size: 1,
		...
		blockquote: {
			...
			paragraph: {
				shading: { fill: theme.blockquoteBackground },
				border: { left: {
					style: "single",
					size: 20,
					color: theme.blockquote,
		...
		tableHeader: {
			...
			properties: { shading: { fill: theme.tableHeaderBackground } },
			run: { bold: true }
		},
		...
		link: {
			...
			run: {
				color: theme.link,
				underline: theme.linkUnderline ? { type: docx.UnderlineType.SINGLE } : void 0
			}
		},
		strong: {
			inline: true,
...
function createDefaultStyle(theme) {
	return {
		document: {
			run: { size: (theme.bodySize ?? defaultTheme.bodySize ?? 12) * 2 },
			paragraph: { spacing: {
				line: Math.round((theme.lineSpacing ?? defaultTheme.lineSpacing ?? 1) * 240),
				lineRule: "auto"
			} }
		},
		hyperlink: {},
...
const defaultStyle = createDefaultStyle(defaultTheme);
function createDocumentStyle({ theme }) {
	...
	const styles$1 = { ...createDefaultStyle(mergedTheme) };
...
function getHeadingLevel(level) {
	...
	switch (level) {
		case 0: return docx.HeadingLevel.TITLE;
...
function renderText(render, text, attr) {
	...
	if (totalLine > 1) {
		const textNodes = [];
		textNodes.push(...multipleLines.map((line, index) => new docx.TextRun({
			...options,
			text: line,
...
function flatInlineToken(render, token, attr) {
	switch (token.type) {
		...
		case "link": return new docx.ExternalHyperlink({
...
let LO_COMPAT = false;
var MathMatrixElement = class extends docx.XmlComponent {
	constructor(children) {
		super("m:e");
		for (const child of children) this.root.push(child);
	}
};
var MathMatrixRow = class extends docx.XmlComponent {
	constructor(cells) {
		super("m:mr");
		for (const cell of cells) this.root.push(new MathMatrixElement(cell));
	}
};
var MathMatrix = class extends docx.XmlComponent {
	constructor(rows) {
		super("m:m");
...
function mathmlToDocxChildren(mathml, opts) {
	const mathNode = findFirst(new fast_xml_parser.XMLParser({
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		if (tag === "munderover" || tag === "munder" || tag === "mover") {
			...
			if (opText.includes("∑")) {
				...
				else out.push(new docx.MathSum({
					children: base,
					subScript: lower,
					superScript: upper
				}));
			...
			if (opText.includes("∫")) {
				if (LO_COMPAT) out.push(...naryAsSubSup("∫", lower, upper, base));
				else out.push(new docx.MathIntegral({
					children: base,
					subScript: lower,
					superScript: upper
				}));
		...
		if (tag === "msubsup") {
			...
			if (tagName(base) === "mo") {
				...
				if (op.includes("∑")) {
					out.push(...LO_COMPAT ? naryAsSubSup("∑", lower, upper, body) : [new docx.MathSum({
						children: body,
						subScript: lower,
						superScript: upper
					})]);
				...
				if (op.includes("∫")) {
					out.push(...LO_COMPAT ? naryAsSubSup("∫", lower, upper, body) : [new docx.MathIntegral({
						children: body,
						subScript: lower,
						superScript: upper
					})]);
...
function walkNode(node) {
	...
	switch (tag) {
		...
		case "msqrt": {
			...
			return [new docx.MathRadical({ children: walkNode(body) })];
		}
		case "mroot": {
		...
		case "mtable": {
			...
			if (LO_COMPAT) {
				const parts = [];
				parts.push(new docx.MathRun("["));
				rows.forEach((row, ri) => {
					if (ri > 0) parts.push(new docx.MathRun("; "));
					childrenOf(row).filter((c) => tagName(c) === "mtd").forEach((cell, ci) => {
						if (ci > 0) parts.push(new docx.MathRun(", "));
...
function textFrom(nodes) {
	...
	return texts ? [new docx.MathRun(texts)] : [];
}
...
function renderBlock$2(render, block, attr) {
	switch (block.type) {
		case "space": return new docx.Paragraph({
			text: "",
			style: classes.Space
		});
		case "code": {
			...
			if (lang && /^(math|latex|katex)$/.test(lang)) {
				...
				if (render.options?.math?.engine === "katex") try {
					const children = mathmlToDocxChildren(katex.default.renderToString(tex, {
						output: "mathml",
						throwOnError: false,
						displayMode: true,
						...render.options.math?.katexOptions || {}
					}), { libreOfficeCompat: !!render.options.math?.libreOfficeCompat });
					if (children && children.length) return new docx.Paragraph({
						children: [new docx.Math({ children })],
		...
		case "hr": return new docx.Paragraph({
			text: "",
			thematicBreak: true,
			style: classes.Hr
		});
		...
		case "def": return new docx.Paragraph({
			text: block.title,
			style: classes.Def
		});
...
function renderInline$1(render, token, attr) {
	return new docx.FootnoteReferenceRun(token.id);
}
...
function renderInline(render, token) {
	...
	if (render.options?.math?.engine === "katex") try {
		const children = mathmlToDocxChildren(katex.default.renderToString(text, {
			output: "mathml",
			throwOnError: false,
			displayMode: !!token.displayMode,
			...render.options.math?.katexOptions || {}
		}), { libreOfficeCompat: !!render.options.math?.libreOfficeCompat });
		if (children && children.length) return new docx.Math({ children });
	} catch {}
	let parsedText = parseLatexToText(text);
	if (!parsedText) parsedText = text;
	if (!parsedText) return new docx.TextRun(text || "");
	return new docx.Math({ children: [new docx.MathRun(parsedText)] });
}
function renderBlock(render, token) {
	...
	if (render.options?.math?.engine === "katex") try {
		...
		if (children && children.length) return new docx.Paragraph({ children: [new docx.Math({ children })] });
	} catch {}
	let parsedText = parseLatexToText(text);
	if (!parsedText) parsedText = text;
	if (!parsedText) return new docx.Paragraph({ children: [new docx.TextRun(text || "")] });
	return new docx.Paragraph({ children: [new docx.Math({ children: [new docx.MathRun(parsedText)] })] });
}

//#endregion
//#region src/extensions/index.ts
function useExtensions(render) {
	const lexer = new marked.Lexer(render.options);
...
function loadImage(src) {
	if (isHttp(src)) return new Promise((resolve, reject) => {
		(src.startsWith("https") ? node_https.default : node_http.default).get(src, (res) => {
			const chunks = [];
			res.on("data", (chunk) => {
				chunks.push(chunk);
			});
			res.on("end", () => {
				...
				(0, image_size.disableTypes)(["svg", "webp"]);
				resolve(buffer);
			});
			res.on("error", (err) => {
...
//#endregion
exports.MarkdownDocx = MarkdownDocx;
exports.Packer = docx.Packer;
exports.default = markdownDocx;
exports.markdownDocx = markdownDocx;
